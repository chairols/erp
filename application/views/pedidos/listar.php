<div class="box">
    <div class="box-header">
        <form method="GET" action="/pedidos/listar/" class="input-group input-group-sm col-lg-12">
            <div class="row">
                <div class="form-group col-lg-4">
                    <input class="form-control" name="cliente" id="cliente" placeholder="Cliente" type="text" value="<?=$this->input->get('cliente')?>">
                </div>
                <div class="form-group col-lg-4">
                    <input class="form-control col-lg-3" name="articulo" id="articulo" placeholder="Artículo" type="text" value="<?= $this->input->get('articulo') ?>">
                </div>
                <div class="form-group col-lg-2">
                    <button type="submit" class="btn btn-primary">Buscar</button>
                </div>
            </div>
        </form>
        <div class="box-tools">
            <ul class="pagination pagination-sm no-margin pull-right">
                <?= $links ?>
            </ul>
        </div>
    </div>
    <div class="box-body no-padding">
        <?php foreach ($pedidos as $pedido) { ?>
            <div class="row listRow listRow2 txC">
                <div class="col-lg-4 col-md-5 col-sm-5 col-xs-3">
                    <div class="listRowInner">
                        <span class="listTextStrong"><?=$pedido['cliente']['cliente']?></span>
                        <span class="smallTitle">(ID: <?=$pedido['cliente']['idcliente']?>)</span>
                    </div>
                </div>
                <div class="col-lg-3 col-md-2 col-sm-2 col-xs-3">
                    <div class="listRowInner">
                        <span class="smallTitle">Total</span>
                        <span class="listTextStrong">
                            <?php $total = 0; ?>
                            <?php foreach($pedido['items'] as $item) {
                                $total += ($item['cantidad'] * $item['precio']);
                            } ?>
                            <span class="label label-brown"><?=$pedido['moneda']['simbolo']?> <?=number_Format($total, 2)?></span>
                        </span>
                    </div>
                </div>
                <div class="col-lg-1 col-md-2 col-sm-3 col-xs-3">
                    <div class="listRowInner">
                        <span class="smallTitle">Fecha</span>
                        <span class="listTextStrong">
                            <span class="label label-info"><?=$pedido['fecha_formateada']?></span>
                        </span>
                    </div>
                </div>
                <div class="col-lg-1 col-md-1 col-sm-1 hideMobile990"></div>
                <div class="animated DetailedInformation col-md-12">
                    <div class="list-margin-top">
                        <?php foreach($pedido['items'] as $item) { ?>
                        <div class="row bg-gray" style="padding: 5px">
                            <div class="col-lg-4 col-sm-5 col-xs-12">
                                <div class="listRowInner">
                                    <span class="listTextStrong"><?=$item['articulo']['articulo']?></span>
                                    <span class="smallTitle"><?=$item['idarticulo']?> (<?=$item['marca']?>)</span>
                                </div>
                            </div>
                            <div class="col-sm-2 col-xs-12">
                                <div class="listRowInner">
                                    <span class="smallTitle">Precio</span>
                                    <span class="listTextStrong">
                                        <span class="label label-brown"><?=$pedido['simbolo']?> <?=$item['precio']?></span>
                                    </span>
                                </div>
                            </div>
                            <div class="col-sm-2 col-xs-12">
                                <div class="listRowInner">
                                    <span class="smallTitle">Precio en Dólares</span>
                                    <span class="listTextStrong">
                                        <span class="label label-success">U$S <?=number_format($item['precio']/$pedido['cotizacion_dolar']['valor']*$pedido['cotizacion_moneda']['valor'], 2)?></span>
                                    </span>
                                </div>
                            </div>
                            <div class="col-sm-3 col-xs-12">
                                <div class="listRowInner">
                                    <span class="smallTitle">Cantidad Pendiente</span>
                                    <span class="listTextStrong">
                                        <span class="label bg-navy"><?=$item['cantidad_pendiente']?></span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <?php } ?>
                    </div>
                </div>
                <div class="listActions flex-justify-center">
                    <div>
                        <span class="roundItemActionsGroup">
                            <a class="hint--top hint--bounce hint--info" href="/pedidos/modificar/<?=$pedido['idpedido']?>/" aria-label="Modificar">
                                <button class="btn btn-primary" type="button">
                                    <i class="fa fa-edit"></i>
                                </button>
                            </a>
                        </span>
                    </div>
                </div>
            </div>
        <?php } ?>
    </div>
    <div class="box-footer clearfix">
        <div class="pull-left">
            <strong>Total <?= $total_rows ?> registros.</strong>
        </div>
        <div class="box-tools">
            <ul class="pagination pagination-sm no-margin pull-right">
                <?= $links ?>
            </ul>
        </div>
    </div>
</div>